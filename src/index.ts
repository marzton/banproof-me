...
async function handleContactForm(request: Request, env: Env): Promise<Response> {
  ...
  try {
    await env.PLATFORM_DB
      ...
      .bind(id, formType, name, email, message, request.headers.get('CF-Connecting-IP') ?? null)
      .run();
  } catch (e) {
...
async function handlePoASubmit(request: Request, env: Env): Promise<Response> {
  ...
  try {
    body = await request.json<WorkflowParams>();
  } catch {
...
export default {
  ...
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (pathname === '/health') {
      return new Response(
        JSON.stringify({ ok: true, service: 'banproof-me', env: env.ENV }),
    ...
    if (pathname === '/api/poa/submit' && request.method === 'POST') {
